const CACHE='sku-decision-auth-v1-4';
...
self.addEventListener('fetch',event=>{const r=event.request;if(r.method!=='GET')return;const u=new URL(r.url);if(u.origin!==self.location.origin)return;event.respondWith((async()=>{try{const resp=await fetch(r);const patched=(r.mode==='navigate'||u.pathname.endsWith('/index.html')||u.pathname.endsWith('/'))?await patchHtml(resp):resp;const copy=patched.clone();caches.open(CACHE).then(c=>c.put(r,copy));return patched;}catch(_){const cached=await caches.match(r)||await caches.match('./index.html');return cached?((r.mode==='navigate'||u.pathname.endsWith('/index.html')||u.pathname.endsWith('/'))?patchHtml(cached):cached):Response.error();}})());});
